...
/**
 * Escape dynamic text before placing it inside template HTML.
 */
...
export function bookingOverlapsDateRange(booking, checkin, checkout) {
  if (!booking || booking.status === 'Cancelled') return false;
  return booking.check_in < checkout && booking.check_out > checkin;
}

export async function getUnavailableRoomIds(checkin, checkout) {
  if (!isISODate(checkin) || !isISODate(checkout) || checkout <= checkin) return new Set();
  ...
  return new Set((data || []).filter(b => bookingOverlapsDateRange(b, checkin, checkout)).map(b => b.room_id));
}

export async function isRoomAvailableForDates(roomId, checkin, checkout) {
  const unavailable = await getUnavailableRoomIds(checkin, checkout);
