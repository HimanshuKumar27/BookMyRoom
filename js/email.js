// ============================================================
// EmailJS Integration — Booking Confirmation Emails
// ============================================================
// INSTRUCTIONS:
//   1. Sign up at https://www.emailjs.com (free plan: 200 emails/month)
//   2. Create an Email Service (e.g., Gmail, Outlook)
//   3. Create an Email Template with variables:
//      - {{guest_name}}, {{email}}, {{room_name}}, {{check_in}},
//        {{check_out}}, {{nights}}, {{total_cost}}, {{booking_id}}
    ...
    script.onload = () => {
      ...
      try {
        if (window.emailjs) {
          ...
          reject(new Error("EmailJS object not found after load"));
        }
      } catch (err) {
        reject(err);
      }
 ...
 * @param {Object} bookingData - includes guest_name, email, room_name, check_in, check_out, nights, total_cost, booking_id
 */
export async function sendBookingConfirmation(bookingData) {
  // We use a timeout to ensure the UI doesn't hang forever if EmailJS is slow
  ...
  const sendPromise = (async () => {
    try {
      ...
      console.log("Preparing guest confirmation email...");
      emailPromises.push(
        window.emailjs
      ...
      if (ENV.ADMIN_EMAIL) {
        ...
        const adminParams = {
          ...
          email: ENV.ADMIN_EMAIL, // For templates using {{email}}
          admin_email: ENV.ADMIN_EMAIL, // For templates using {{admin_email}}
          reply_to: bookingData.email, // Allow admin to reply to guest directly
        ...
        emailPromises.push(
          window.emailjs
            .send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, adminParams)
            .then((res) => {
